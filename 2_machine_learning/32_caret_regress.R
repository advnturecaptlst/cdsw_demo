...
#  ## Because the data is the second value, have to use
# the pipe operator differently where the the object to the left
# of the %>% replaces the '.' (dot) in the statement 
model <- chess_train %>% train(weight ~ base_diameter, data = ., method = "lm")
...
# `library()` command.
# You can find the method shorthand, e.g. "rf" is randomForest()
...
R2(test_pred, chess_test$weight)
# R2 on the training set is only slightly higher than test set
# so it's unlikely that the model is overfitting
